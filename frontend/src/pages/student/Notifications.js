...
export default function StudentNotifications() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {notifications.length > 0 && (
        <button
          className="mb-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 w-full sm:w-auto"
          onClick={async () => {
            try {
              await apiRequest('/notification/read-all', { method: 'PATCH' });
              // Mark all notifications as read in the UI
      ...
      {notifications.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {notifications.map(n => (
            <div key={n._id} className={`border p-4 rounded-lg shadow-lg ${n.read ? "bg-gray-50" : "bg-white"}`}>
              <div className="font-semibold mb-2">{n.message}</div>
              <div className="text-sm text-gray-500 mb-2">Type: {n.type}</div>
              <div className="text-sm text-gray-500 mb-2">Created At: {n.createdAt ? new Date(n.createdAt).toLocaleString() : ''}</div>
              <div className="mb-2">
                <span className={`text-xs ${n.read ? "text-green-600" : "text-red-600"}`}>
                  {n.read ? 'Read' : 'Unread'}
                </span>
              </div>
              <div className="text-center">
                {!n.read && (
                  <button
                    className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs w-full sm:w-auto"
                    onClick={() => markAsRead(n._id)}
                  >
                    Mark as Read
                  </button>
